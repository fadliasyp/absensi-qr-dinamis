...
| Export database peserta PDF | PARTIAL | Export per kelompok dan gabungan seluruh kelompok mendukung pilihan 1–6 field inti/dinamis aktif, peserta aktif, kolom nomor, A4 potret dengan baris rapat, dan highlight jumlah gender. Export gabungan memuat catatan jumlah Status Kesibukan tanpa bergantung pada kolom tabel yang dipilih; sintaks/regression test lulus tetapi output Supabase nyata belum diuji. |
...
- Export PDF hanya menerima field inti atau field dinamis aktif yang valid, maksimal enam field, dan hanya memuat peserta aktif dari kelompok terpilih atau seluruh kelompok pada export gabungan.
